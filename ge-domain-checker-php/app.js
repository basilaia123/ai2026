...
const scoreSortEl = document.getElementById("scoreSort");
...
const API_ENDPOINT = "api/check.php";
...
function sortResultsByScore(results) {
  const mode = scoreSortEl?.value || "none";
  if (mode === "none") {
    return results;
  }
  ...
  sorted.sort((a, b) => {
    const scoreA = typeof a.premiumScore === "number" ? a.premiumScore : null;
    const scoreB = typeof b.premiumScore === "number" ? b.premiumScore : null;
...
function refreshResultsView() {
  const filtered = getFilteredResults(latestResults);
  visibleResults = sortResultsByScore(filtered);
...
function handleGenerate() {
  const mode = generationModeEl?.value || (allowSpecialCharsEl?.checked ? "extended" : "letters");
  const allowSpecialChars = mode === "extended";
...
scoreSortEl?.addEventListener("change", () => {
  refreshResultsView();
  statusEl.textContent = `სორტირება განახლდა. ნაჩვენებია ${visibleResults.length} შედეგი.`;
});
